...
void *sync_start(void *arg)
{
...
#ifdef __SWITCH__
    {
        // may invoke game play-state callbacks; needs the fake TLS cookie
        extern void pthr_ensure_fake_tls(void);
    ...
    for (;;) {
        ...
        while (combinedMask) {
            ...
            (void) attributesMask; // only consumed under USE_SNDFILE below
