...
/** \brief Non-blocking exclusive lock attempt; true on success. The Switch
 *  port's stop/clear/destroy fast paths use it to detect "mixer not
 *  mid-pass" without the ABBA risk of a blocking lock.
 */

bool object_trylock_exclusive(IObject *this)
{
    if (0 != pthread_mutex_trylock(&this->mMutex))
        return false;
#ifdef USE_DEBUG
    this->mOwner = pthread_self();
    this->mFile = __FILE__;
