...
// non-blocking exclusive lock attempt (Switch port); true on success.
// NOT renamed by the USE_DEBUG macros below -- callers pair it with
// object_unlock_exclusive, which handles the debug owner bookkeeping.
extern bool object_trylock_exclusive(IObject *this);
