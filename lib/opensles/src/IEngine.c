...
#endif
        *pPlayer = NULL;
        // Detect a streamed (music) source. ANDROIDFD = the engine hands us a
        // file descriptor + region (into the OBB) and expects the platform to
        // decode the MP3. Capture it now; we attach our MP3 decoder once the
        // buffer queue is allocated below. SFX use a PCM buffer queue (handled
        // by the normal path).
        int mp3_fd = -1; long long mp3_off = 0, mp3_len = 0;
        SLboolean is_mp3 = SL_BOOLEAN_FALSE;
        if (pAudioSrc != NULL && pAudioSrc->pLocator != NULL) {
            SLuint32 lt = *(SLuint32 *)pAudioSrc->pLocator;
            if (lt == SL_DATALOCATOR_ANDROIDFD) {
                SLDataLocator_AndroidFD *afd = (SLDataLocator_AndroidFD *)pAudioSrc->pLocator;
                mp3_fd = (int)afd->fd;
                mp3_off = (long long)afd->offset;
                mp3_len = (long long)afd->length;
                is_mp3 = SL_BOOLEAN_TRUE;
            }
        }
        unsigned exposedMask;
        ...
        if (SL_RESULT_SUCCESS == result) {
            ...
            if (NULL == this) {
                ...
                do {
                    ...
                    // Default to FULL volume. construct() zero-inits mGains, and
                    // the track copies the player's gain when it starts playing;
                    // a player the engine plays WITHOUT first calling SetVolume
                    // would otherwise be mixed at gain 0 (silent) -- which muted
                    // ~9/10 of SFX (and the music between its fades). SL's default
                    // volume is 0 mB = full, so seed gain 1.0 here.
                    this->mGains[0] = 1.0f;
                    this->mGains[1] = 1.0f;
#endif
                    ...
                    case SL_DATALOCATOR_ANDROIDSIMPLEBUFFERQUEUE:
                        ...
                        break;
                    default:
                        // streamed MP3 (ANDROIDFD) gets a buffer queue we feed
                        // from the decoder; anything else stays buffer-less.
                        this->mBufferQueue.mNumBuffers = is_mp3 ? MP3STREAM_NUM_BUFFERS : 0;
                        break;
                    }
                    ...
                    //if (0 != this->mBufferQueue.mNumBuffers) {
                        ...
                        IBufferQueue_InitHeaders(&this->mBufferQueue);
                        this->mBufferQueue.mFront = this->mBufferQueue.mArray;
                        this->mBufferQueue.mRear = this->mBufferQueue.mArray;
                        //}

                        // streamed MP3 (music): attach the decoder to feed the
                        // buffer queue we just allocated. On failure the player
                        // is left silent (no regression vs. before).
                        if (is_mp3) {
                            SLresult mr = mp3stream_setup(this, mp3_fd, mp3_off, mp3_len);
                            if (SL_RESULT_SUCCESS != mr)
                                debugPrintf("SL: mp3stream_setup failed 0x%x -> player silent\n",
                                            (unsigned)mr);
                        }
                ...
                if (SL_RESULT_SUCCESS != result) {
                    ...
                    *pPlayer = &this->mObject.mItf;
                    // streamed MP3: now that the player is published, pre-fill its
                    // buffer queue so playback starts with data.
                    if (is_mp3)
                        mp3stream_prime(this);
#ifdef SYBERIA
...
#endif
#ifdef USE_SDL
                // the audio device is already unpaused at SDL_open; just
                // connect the new output mix so the callback starts mixing it
