...
#ifndef __SWITCH__
#include <psp2/kernel/processmgr.h> // sceKernelGetProcessTimeLow (Vita)
#endif
    ...
    case SL_PLAYSTATE_PLAYING:
        ...
        for (;; interface_cond_wait(this)) {
            ...
            case (SL_PLAYSTATE_PLAYING  << 2) | SL_PLAYSTATE_STOPPED:
                {
                // Stopping a voice is the game's most frequent audio call
                // (every SFX end); the stock code parks the caller in
                // STOPPING until the mixer's next pass acknowledges. Fast
                // path (Switch port): trylock the output mix; success means
                // the mixer is not mid-pass, so apply its STOPPING->STOPPED
                // transition (track_check) inline with zero wait. (Trylock,
                // not lock: the mixer takes outputmix->player.)
                Track *track = (NULL != audioPlayer) ? audioPlayer->mTrack : NULL;
                if (NULL == track) {
                    // no mixer track: no pass will ever acknowledge (the
                    // stock handshake would hang forever); stop directly
                    this->mState = state;
                    break;
                }
                COutputMix *outputMix = CAudioPlayer_GetOutputMix(audioPlayer);
                if (object_trylock_exclusive(&outputMix->mObject)) {
                    // mirror of track_check's SL_PLAYSTATE_STOPPING case
                    ...
                    const BufferHeader *oldFront = audioPlayer->mBufferQueue.mFront;
                ...
                // mixer is mid-pass (rare): stock handshake, bounded by one
                // mixer period; timed so debug.log shows the residual cost
                stop_wait_t0 = sceKernelGetProcessTimeLow();
                if (0 == stop_wait_t0)
                    stop_wait_t0 = 1;
        ...
        if (0 != stop_wait_t0) {
            ...
            static int stop_waits = 0;
            int n = ++stop_waits;
            if (n <= 12 || (n % 500) == 0)
                debugPrintf("SL: SetPlayState(STOPPED) waited %u us for mixer (wait #%d)\n",
                            sceKernelGetProcessTimeLow() - stop_wait_t0, n);
        }
#else
